...
namespace Femto3D{
  ...
  hBVH::BVH(const hvector< AABB > & leaves) {
    ...
    if constexpr (print_timings) std::cout << "allocation: " << stopwatch.elapsed() << '\n';

    if constexpr (print_timings) stopwatch.start();
    float big = 1.0e10; 
    global = thrust::reduce(
      leaves.begin(), leaves.end(),
      AABB(big, big, big, -big, -big, -big),
      [](AABB A, AABB B){ return union_of(A, B); });
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "global bounding box: " << stopwatch.elapsed() << '\n';

    if constexpr (print_timings) stopwatch.start();
    int bits_per_dimension = (sizeof(code_t) * 8) / 3;
    int divisions_per_dimension = 1 << bits_per_dimension;

    vec3f scale = float(divisions_per_dimension - 1) / global.widths();

    hvector < hBVH::code_t > codes(num_leaves);

    #pragma omp parallel for
    for (int i = 0; i < num_leaves; i++) {
      vec3f u = (leaves[i].center() - global.min()) * scale;

      ids[i] = i;
      codes[i] = morton::encode(static_cast< code_t >(u[0]), 
                                static_cast< code_t >(u[1]), 
                                static_cast< code_t >(u[2]));
    }
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "calculate morton codes: " << stopwatch.elapsed() << '\n';

    if constexpr (print_timings) stopwatch.start();
    thrust::stable_sort_by_key(codes.begin(), codes.end(), ids.begin());
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "sort morton codes: " << stopwatch.elapsed() << '\n';

    if constexpr (print_timings) stopwatch.start();
    thrust::gather(ids.begin(), ids.begin() + num_leaves, leaves.begin(), boxes.begin());
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "rearrange leaves: " << stopwatch.elapsed() << '\n';
    ...
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "radix tree: " << stopwatch.elapsed() << '\n';
    ...
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "bounding boxes: " << stopwatch.elapsed() << '\n';

  }

  hvector< int2 > hBVH::all_intersections_with(const hvector< vec3 > & points) {
    ...
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "preallocation: " << stopwatch.elapsed() << "s" << std::endl;
    ...
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "traversal: " << stopwatch.elapsed() << "s" << std::endl;
    ...
    if constexpr (print_timings) stopwatch.stop();
    if constexpr (print_timings) std::cout << "merge: " << stopwatch.elapsed() << "s" << std::endl;
